...
def main():
    ...
    embeddings_path = '../embeddings/dda_gcn_no_train_embeddings.pt'
    results_path = '../results/gcn_no_train'
    ...
    for run in range(num_runs):
        ...
        # Model Setup
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = DDINetworkRegression(embedding_dim=64, hidden_dim=1024, output_dim=512, dropout_rate=0.1).to(device)
