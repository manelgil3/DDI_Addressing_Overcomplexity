...
def test_metrics(model, loader, device, results_path):
    ...
    plt.plot(line, slope * line + intercept, 'b-', label="Regression Line")

    # Error Region
    residuals = all_predictions - (slope * all_labels + intercept)
    std_dev = np.std(residuals)
    plt.fill_between(line, slope * line + intercept - std_dev, slope * line + intercept + std_dev, color='blue', alpha=0.2, label="Error Region")
    ...
    # Annotations
    plt.text(x_min + 0.5, x_max - 1, f"Pearson: {pearson_corr:.3f}\nRMSE: {rmse:.3f}", fontsize=10, bbox=dict(facecolor='white', alpha=0.5))

    plt.xlabel("Ground Truth (log_2)")
    plt.ylabel("Prediction (log_2)")
...
def main():
    ...
    embeddings_path = '../embeddings/dda_mfps_embeddings.pt'
    results_path = '../results/mfps'
    ...
    for run in range(num_runs):
        ...
        # Model Setup
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = DDINetworkRegression(embedding_dim=2048, hidden_dim=1024, output_dim=512, dropout_rate=0.1).to(device)
